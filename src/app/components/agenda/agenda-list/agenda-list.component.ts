...
import { faPencilAlt, faSearch, faPlus } from '@fortawesome/free-solid-svg-icons';
...
export class AgendaListComponent implements OnInit {
  ...
  dataSource: any;
  displayedColumns: string[] = ['prontuario', 'nome', 'telefone', 'celular', 'hora', 'convenio', 'primeira_vez', 'compareceu', 'pagou', 'idade', 'observacao']
    ...
    this.agendaservice.getAgendamentos(this.medicoId, this.data_agendamento).subscribe(
      data => {
        if (Object.keys(data).length === 0) {
          ...
          this.convertTime();
          this.buildTable();
          this.compareDates(this.agendamentos[0].data.toString());
