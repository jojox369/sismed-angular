...
export class FuncionarioDetailsComponent implements OnInit {
  ...
  createForm() {
    ...
    this.formFuncionario = this.fb.group({
      ...
      nome: [{ value: this.funcionario.nome, disabled: true }, Validators.required],
  ...
  */
  ...
  update(frm: FormGroup) {
    ...
    if (this.dataInicio !== this.formFuncionario.controls.data_inicio.value) {
      ...
      this.log.descricao = 'ALTERAÇÃO NA DATA DE CONTRATAÇÃO DO FUNCIONÁRIO ' + this.funcionario.nome + '. DA DATA ' +
        this.formatDate(this.dataInicio) + ' PARA A DATA ' + this.formatDate(this.formFuncionario.controls.data_inicio.value);
    ...
    if (this.dataTermino !== this.formFuncionario.controls.data_termino.value) {
      if (this.dataTermino !== null) {
        ...
        this.log.funcionario = this.user.id;
        this.log.evento = 'EDIÇÃO';
        this.log.descricao = 'ALTERAÇÃO NA DATA DE DISPENSA DO FUNCIONÁRIO ' + this.funcionario.nome + '. DA DATA ' +
          this.formatDate(this.dataTermino) + ' PARA A DATA ' + this.formatDate(this.formFuncionario.controls.data_termino.value);
        this.logService.save(this.log).subscribe(
        ...
        this.log.funcionario = this.user.id;
        this.log.evento = 'EDIÇÃO';
        this.log.descricao = 'ALTERAÇÃO NA DATA DE DISPENSA DO FUNCIONÁRIO ' + this.funcionario.nome + '. DE DATA DE DISPENSA NÃO CADASTRADA PARA A DATA ' +
          this.formatDate(this.formFuncionario.controls.data_termino.value);
        this.logService.save(this.log).subscribe(
    ...
    this.funcionarioService.updateFuncionario(this.formFuncionario.value).subscribe(
      data => {
        this.funcionario = data;
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'true') {
        ...
        this.userService.getUsers().subscribe(
            ...
            for (const user of data) {
              if (user.username === this.funcionario.cpf) {
                this.userService.deleteUser(user.id).subscribe(
                  data => {
                    this.funcionarioService.deleteFuncionario(this.funcionarioId).subscribe(
                      (data) => {
                        ...
                        this.log.descricao = 'EXCLUSÃO DO FUNCIONÁRIO ' + this.funcionario.nome;
                        this.logService.save(this.log).subscribe(
                          data => {
                            this.funcionarioService.message = 'Funcionário(a) excluido(a) com sucesso!';
                  ...
                  error => {
                    console.log(error);
                    this.buildMessage('Erro ao tentar excluir as credenciais do funcionario(a)', 0);
                  }
                );
              }
            }
          ...
          error => {
            console.log(error);
            this.buildMessage('Erro ao tentar excluir o funcionario', 0);
          }
        );
  ...
  registerTiposConvenio() {
    ...
    const dialogRef = this.dialog.open(FuncionarioTipoConvenioRegisterComponent, {
      height: '500px',
      width: '600px',
